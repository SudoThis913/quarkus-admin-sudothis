// File: src/main/java/com/sudothis/controller/UserController.java
...
public class UserController {
    ...
    @Path("/register")
    @Transactional
    public Response registerUser(RegistrationRequest request, @Context HttpServletRequest httpRequest) {
        ...
        if (attempts >= 20) {
            String ttlKey = redis.get(rateKey + ":ttl");
            String ttlMessage = (ttlKey != null) ? ttlKey : "60";
            return Response.status(Response.Status.TOO_MANY_REQUESTS)
                    .entity("Rate limit exceeded. Try again in ~" + ttlMessage + " seconds.")
                    .build();
        }
        ...
        User user = userService.registerUser(request.password.toCharArray(), request.username, request.email);

        String token = UUID.randomUUID().toString();
        String redisKey = "verify_email:" + token;
        redis.setex(redisKey, EMAIL_TOKEN_TTL.getSeconds(), String.valueOf(user.getId()));
        redis.setex("verify_lookup:uid:" + user.getId(), EMAIL_TOKEN_TTL.getSeconds(), token);
        redis.setex("verify_cooldown:" + user.getId(), EMAIL_RESEND_LIMIT.getSeconds(), "1");

        sendStubValidationEmail(user, token);

        return Response.status(Response.Status.CREATED).entity("User registered. Please check your email to verify your account.").build();
    }
    ...
    public Response resendEmailVerification(ResendRequest request) {
        ...
        String cooldownKey = "verify_cooldown:" + user.getId();
        if (redis.get(cooldownKey) != null) {
            return Response.status(Response.Status.TOO_MANY_REQUESTS).entity("Please wait before requesting another verification email.").build();
        }

        String priorTokenKey = "verify_lookup:uid:" + user.getId();
        String oldToken = redis.get(priorTokenKey);

        if (oldToken != null) {
            redis.setex("verify_email:" + oldToken, 1, "");
        }

        String newToken = UUID.randomUUID().toString();
        redis.setex("verify_email:" + newToken, EMAIL_TOKEN_TTL.getSeconds(), String.valueOf(user.getId()));
        redis.setex(priorTokenKey, EMAIL_TOKEN_TTL.getSeconds(), newToken);
        redis.setex(cooldownKey, EMAIL_RESEND_LIMIT.getSeconds(), "1");

        sendStubValidationEmail(user, newToken);
